...
class Dice(ClassyMeter):
    ...
    def validate(self, model_output, target):
        ...
        logging.info(f"Number of classes in the model output is {torch.max(model_output)+1} but the dataloader supply ony {self.num_classes} as as ground trth. The meter is followed from {source}")
