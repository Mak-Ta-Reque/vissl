...
from meter_test_utils import  SegmentaionMeterTest
from vissl.meters.hd95_meter import HD95


class TestRecallAtKListMeter(SegmentaionMeterTest):
    def test_hd95_meter_registry(self):
        meter = build_meter(
            {
                "name": "hd95",
                "encoded_classes": [0, 1, 2],
                "softmax": True,
            }
        )
        self.assertTrue(isinstance(meter, HD95))
    ...
    def test_single_meter_update_and_reset(self):
        ...
        meter = HD95(meters_config=meter_attribute)

        # Batchsize = 3, num classes = 3, score is probability of class
        model_output = torch.tensor(  # 1 x 3 x 3 x 3
            [[[
                [0.2, 0.4, 0.5],  # top-1: 1/2, top-2: 1/2
        ...
        expected_value = {'hd95 score': {0: 0.0, 1: 0.0, 2: 0.0}}
